...
def editFile(path="output",fileName="carmelo.csv"):
	...
	df["SFG%"]=df["SFG"]/df["SFGA"]
	df["SFT%"]=df["SFT"]/df["SFTA"]
	df["S3P%"]=df["S3P"]/df["S3PA"]

	df["STS%"]= df["SPTS"] / (2 * df["SFGA"]+.44*df["SFTA"]) 
	#add a column for Effective Field Goal Percentage, a better metric for a players shooting ability
	df["SEFG%"]=(df["SFG"]+.5*df["S3P"]) / df["SFGA"]

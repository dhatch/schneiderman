...
def main():
    ...
    players["F"] = []
    players["UTIL"] = []
    a = open('../regression/expectedvals', 'r')
    for line in a:
        line = line.split(',')
        p = player(name=line[0], position=line[1], e_v=float(line[2]), var=float(line[3]), diff = float(line[4]), salary=float(line[4]))
    ...
    #set variances
    for i in range(1, 6):
        a = open('../regression/resid'+position_order[i]+'.csv', 'r')
        variance_dict = {} #key player name, key (sum squared resid, num_games)
        for line in a:
            line = line.split(',')
